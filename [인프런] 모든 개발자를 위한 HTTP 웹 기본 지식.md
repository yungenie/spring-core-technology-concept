...
### 대표적인 응답 코드
  ...
  - 400 Bad Request (잘못된 요청) : 잘못된 문법 또는 요청으로 인해 서버가 요청을 이해할 수 없는 경우
  - 403 Forbidden (거부됨): 권한이 없어 요청에 실패했음
  - 404 Not Found (찾을 수 없음): 서버가 요청받은 리소르르 찾을 수 없는 경우. '서버를 찾을 수 없다'는 게 아니라, 서버 통신은 되지만 요청한 바를 찾을 수 없다는 뜻
  - 500 Internal Server Error (내부 서버 에러) : 서버가 처리 방법을 모르는 상황과 마주쳤지만, 서버가 처리 방법을 알 수 없을 경우. 서버에 오류가 발생해 작업을 수행할 수 없을 때 나타나는 코드.
  - 502 Bad Gateway (불량 게이트웨이) : 서버가 요청을 처리하는 데 필요한 응답을 얻기 위해 게이트웨이로 작업하는 동안 잘못된 응답을 받았다는 뜻. 
  - 503 Servic Unavailable (일시적으로 이용할 수 없음) : 서버가 요청을 처리할 준비가 되지 않았음. 보통은 유지보수를 위해 작동을 중단했거나 과부하 상태가 된 서버로 인해 발생하는 코드
  - 504 Gateway Timeout (게이트웨이 시간 초과) : 게이트웨이가 연결된 서버로부터 응답을 적절한 시간 내에 받을 수 없다는 뜻 
